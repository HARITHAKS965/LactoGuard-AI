...
async function generateExcelReport() {
  ...
  workbook.creator = 'LactoGuard Security Auditor';
  workbook.created = new Date();

  // Primary sheet matching exact Selenium & Appium Test Results layout
  const worksheet = workbook.addWorksheet('Vulnerability Check Results');
  worksheet.columns = [
  ...
  testResults.forEach(r => {
    const row = worksheet.addRow({
      testId: r.testId,
      testName: r.testName,
      category: r.category || 'Security & Vulnerability Audit',
      status: r.status,
  ...
  const filePath = path.join(reportsDir, 'vulnerability-report.xlsx');
