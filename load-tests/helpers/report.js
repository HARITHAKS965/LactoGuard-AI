...
async function generateExcelReport() {
  ...
  workbook.creator = 'LactoGuard Load Performance Engineer';
  workbook.created = new Date();

  // Primary sheet matching exact Selenium/Appium/Vulnerability Test Results layout
  ...
  testResults.forEach(r => {
    const row = worksheet.addRow({
      testId: r.testId,
      testName: r.testName,
      category: r.category || 'Load & Performance Automation',
      status: r.status,
  ...
  const filePath = path.join(reportsDir, 'load-test-report.xlsx');
